...
summed values of the LG mass, since MW+M31 mass estimates often cannot measure
the full extent of the distribution of dark matter in the LG, and thus must
...
M31 and the MW with respect to the barycenter, they find a LG mass of 
...
recovered total mass by $0.6\times10^{12}~\Msun$.

However, our LG mass estimates are consistent with a number of other recent 
...
\cite{Zhai2020} identified pairs of stellar analogs to the MW and M31, then applied a series of kinematic cuts on the separation, isolation, and velocities of the pair, to determine Local Group analogs in two, $N$-body cosmological simulations, Millenium-WMAP7 and MilleniumII. 
They find stellar and dynamical LG analogs on mostly radial orbits have total masses of $4.4^{+2.4}_{-1.5}\times 10^{12}\Msun$, which is consistent with our findings for each dataset. 
They also find that low-ellipticity orbits (where $v_{\rm rad}\sim v_{\rm tan}$), result in a higher LG mass, M31 mass, and MW mass, reporting $6.6^{+2.7}_{-1.5}\times10^{12}\Msun$, $3.8^{+2.8}_{-1.8}\times10^{12}\Msun$, and $2.5^{+2.2}_{-1.4}\times10^{12}\Msun$, respectively. 
We find that the ellipticity of the orbit decreases dramatically with larger travel velocities, however, in contradiction to \cite{Zhai2020}, we find this leads to lower total LG mass.
...
reflex motion from their velocity measurements.
...
Implementing the Timing Argument and removing the LMC-induced reflex motion,
...
consistent with our finding of a reduction in the mass by $\sim10-20\%$.
...
account for the extensive merger history of the MW in a robust way without
the need to simulate the entire interaction history. 
...
\begin{table*}
  ...
  \begin{tabular}{clc|c}
    ...
    %  &  &  \\
    \hline
    \multirow{4}{*}{\mmto}& Kinematics of M31 Sats & $1.4 \pm 0.4$ ($<$300 kpc) 
    & \cite{Watkins2010}\\
    ...
    \hline
    \multirow{5}{*}{\mmw}&  Kinematics of LG sats & $1.4 \pm 0.3$ ($<$300 kpc) 
    & \cite{Watkins2010}\\
...
However, the both the direction and magnitude of travel velocity are directly 
tied to the inferred mass of the Local Group.
...
LMC--Milky Way system, we do not expect there to be a significant reflex
...
Rectifying the discrepancy between the summed M31+MW masses and the total LG
mass is not straightforward, however, the two approaches are complimentary.
Further constraints on the travel velocity of the MW disk may start pushing to
higher and higher travel velocities (see Section~\ref{sec:vtrav}), which would
place tight upper bound constraints on the total mass of the LG.
Mass constraints on the MW have been consistently finding MW mass of
~$\sim10^{12}\Msun$, if not slightly higher (see for reference
Table~\ref{table:masses} that lists a compilation of selected mass results from
the literature).
If we assume a MW mass of $\sim10^{12}\Msun$, our upper bound limitations from
the Timing Argument may begin placing meaningful constraints on the upper bound
mass of the M31 system.


%%%%%%%%%%%
\subsection{The circularity of the MW--M31 orbit}
The Timing Argument is highly sensitive to the tangential motion of M31, and larger PMs will generically lead to lower eccentricities and higher inferred LG masses. 
Recent \gaia\ PMs of M31 suggest the orbit of MW--M31 is less radial than previously believed~\citep{vdm2019,Salomon2021}.
Neglecting the travel velocity ($\vtrav=0$ in above plots), we find that PMs from \textit{HST}~\citep{vdm2012} are consistent with a highly radial
orbit $e\sim0.96$, while PMs from \textit{Gaia}~\citep{Salomon2021} result in a slightly lower eccentricity of $e\sim0.88$.
As seen in Fig.~\ref{fig:mvsv}, we find that as the travel velocity increases, the inferred eccentricity of the MW--M31 orbit decreases. 
However, contrary to expectations, we find that the inferred LG mass \textit{also} decreases. 
This implies that the velocity contribution to the relative and transverse velocity of M31 is dominant to the change in eccentricity. 

Additionally, studies of the Bolshoi $N$-body cosmological simulation by~\cite{Forero-Romero2013} find that typical LG analogs do not have completely radial orbits. 
As with other studies that find a higher transverse velocity for M31 (and thus a slightly less-than-radial orbit)~\citep{vdm2019,Salomon2021}, we find that the decrease in the eccentricity due to the measured travel velocity makes the LG less eccentric and, thus, more cosmologically typical. 
...
\begin{itemize}
  ...
  measurements of M31~\citep{Li2021,Salomon2021} -- we find a new, completely 
  empirical, measurement for the LG mass of
...
\begin{itemize}
  ...
  halo ($\vtrav=0$). 
  Using the same distance and PMs as in the Timing Argument model 
  ...
  Galactocentric distances $(60-100\kpc)$ will yield larger measurements of the
  travel velocity. 
  If travel velocities are measured to be higher at larger Galactocentric 
  distances, this will result in a decrease in the inferred LG mass by an 
  additional $5-12\%$.
  %
  %
  ...
  As we increase the travel velocity, the orbit becomes rapidly less radial. 
  ...
  makes the LG more typical.
  %
  %
  \item \textbf{Improvements in M31 PM measurements will improve Timing 
  Argument mass precision.}
  We scaled the PMs errors in each dataset by ~6, the expected improvement in
  the proper motion precision between \textit{Gaia} EDR3 and the end of the
  10-year lifetime (see Section~\ref{sec:discussion-futureobs} for more detail),
  and found an expected improvement in the uncertainty on the mass by a factor 
  of $2-2.5\times$.
  %
  %
\end{itemize}
...
It is critical to refine our measurements of the PM of M31 and to measure the
travel velocity of the MW disk with stellar tracers at further Galactocentric
distances.
These endeavors will: 1. further refine estimates of the mass of the LG,
enabling studies to realistically place the LG in a cosmological context; and
2. permit travel velocity measurements at the largest Galactocentric distances
possible, which will establish a firm lower bound for the LG mass
via the Timing Argument and thereby place meaningful limits on the individual
masses of the M31 and MW galaxy.
