...
\citet{Penarrubia2016} studied the impact of the presence of the Large Magellanic Cloud (LMC) on the
...
\begin{table*}
  ...
  \begin{tabular}{clc|c}
    ...
    & Cosmological Sims (likelihood-free inference) & $4.6^{+2.3}_{-1.8}$ & \cite{Lemos2021}\\
    & TA + LMC & 5.6$^{+1.6}_{-1.2}$ & \cite{Benisty2022}\\
    & TA + CB + LMC & 3.4$^{+1.4}_{-1.1}$ & \cite{Benisty2022}\\
    & \textbf{TA + $\boldsymbol{\vtrav}$} & {$\boldsymbol{4.5^{0.8}_{0.6}}$}& \textbf{This work (Chamberlain et al. 2022)}\\
    \hline
    ...
    & Machine Learning (+velocity information) & $2.2-2.5$ & \cite{Villanueva-Domingo2021}\\
    ...
    & Machine Learning & $\sim1.0-1.3$ & \cite{Villanueva-Domingo2021}\\
    & Machine Learning (+velocity information) & $\sim2.3-2.6$ & 
    \cite{Villanueva-Domingo2021}\\
  \hline\hline
  \end{tabular}
  \caption{\label{table:masses}A selected collection of previous mass
  measurements from LG dynamics.}
  %note: added Lemos but they are MW+M31 sum only? do I need to specify? 
\end{table*}
...
inferred LG mass.
\todo{Stellar tracers at large Galactocentric distances may yield measurements
of the apex direction that differ from current measurements.}
