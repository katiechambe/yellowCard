...
Local Group dynamics are dominated by the local gravitational potential where the Hubble flow can be neglected~\cite{}. We construct a Keplerian orbital model where the kinematics of the M31-MW system are completely set by 4 model parameters: (\mlg, $a$, $e$, $\eta$) where \mlg is the mass of the Local Group, and $a$, $e$, and $\eta$ are the semi-major axis, eccentricity, and eccentric anomaly of an orbit of M31 about the MW. The analytic equations for the separation, the elapsed time since pericenter, and the velocity components from the equations of motion for a two body Keplerian interaction are:
...
We used \texttt{pymc3}~\citep{Salvatier2016} to sample from the posterior distribution of model parameters \mlg, $a$, $e$, $\eta$, \kc{and $\alpha$, where $\alpha$ is a parameter that fixes the pole of the orbital plane of M31, and thus establishes the angle between the Keplerian tangential velocity and the proper motion measurements -- did we end up utilizing alpha?}. We set normal priors on the distance to M31 and the Local Group mass, a uniform prior on $\ln(1-e)$, and \kc{an angle prior implemented in the pymc3\_ext package, don't know how to describe this} (see Table~\ref{table:priors} for details).

Details about our pymc3 run like \# chains, steps, etc.
We ran our model with -- chains, for -- steps and with -- tuning steps. 
\begin{table}
  \centering
  \begin{tabular}{cc|c}
    \hline\hline
    Parameter & Prior dist.   & Properties \\\hline
\multirow{2}{*}{\mlg}      & \multirow{2}{*}{$\mathcal{N}$} & $\mu$: $4.5\times10^{12}$\Msun, $\sigma$: $3\times10^{12}$\Msun\\
    & & bounds: $[0.5,20]\times10^{12}$ \Msun\\
  %              &           &           \\
  \multirow{2}{*}{$r$}      & \multirow{2}{*}{$\mathcal{N}$} & $\mu$: 700kpc, $\sigma$: 100kpc\\
    & & bounds: $[100,10000]$kpc\\
  %              &           &           \\ 
  \multirow{1}{*}{$\ln(1-e)$}      & \multirow{1}{*}{$\mathcal{U}$} & [-10,0]\\
  \multirow{1}{*}{$\eta$}      & \multirow{1}{*}{angle} & [$-\pi,\pi$] or [0, $2\pi$]\\
  \hline\hline
  \end{tabular}
  \caption{\label{table:priors}Priors \kc{i do not like this table yet, change Properties to bounds and $\mathcal{N}(4.5e12, 3e12)$ etc }}
\end{table}
...
\kc{Why are we interested in constraining local group mass?}
\kc{What did we find Fig.~\ref{fig:contour}?}
\begin{itemize}
  \item r stays the same 
  \item M decreases for both datasets
  \item eccentricity decreases in our model
  \item why does dataset 1 differ from dataset 2 in r and e? 
\end{itemize}
