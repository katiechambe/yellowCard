...
as `\mwouter'. 
Thus, the position and velocity of M31 with respect to MW can be
...
Then, the position and velocity of M31 as measured in a heliocentric
...
\end{align}
Here $\left|\pos{M31}{\mwouter}\right| = r$ as determined from Eq.~\ref{eq:r}, 
$\vel{M31}{\mwouter}$ is determined
completely by the Keplerian model parameters (through \vrad\ and \vtan), and the
position and velocity of the center of the MW halo as measured from the sun
are $\pos{\mwouter}{\odot}$ and $\vel{\mwouter}{\odot}$, respectively, and are
...
tracks with the center of its dark matter halo. 
In fact, these interactions will introduce an additional reflex motion component 
in observations from the MW disk. 
The LMC's impact, in particular, on the dynamics of the MW disk and inner halo 
have been studied in detail by~\cite[e.g.,][]{Garavito-Camargo2021b, petersen 2021? and garciaz2016?

These works imply that we must account for additional terms in
Equations~\ref{eq:xoffset1} and \ref{eq:voffset1} to account for the reflex
motion of the MW disk with respect to the center of the halo in its unperturbed 
state.
In this study, we will account for the reflex motion of the MW disk in an 
empirically-driven manner, avoiding uncertainties in the LMC orbital 
...
In principle, there is also a term $\vec{M31halo}{M31disk}$, but as no measurements of the differential motion of the disk/inner regions of m31 haven't been measured, we drop this term.  See discussion in Sec.~\ref{sec:discussion-m31}.

\subsubsection{In terms of observables:}
...
\begin{figure*}[htb]
  ...
  \caption{
    ...
    Finally, $\boldsymbol{x}_{\rm travel}$ and $\boldsymbol{v}_{\rm travel}$ 
    are the distance and velocity between the MW disk and the center of the 
    MW halo and represent the measured components of the reflex motions from the composite 
    perturbations of many interactions in the MW halo.
    \todo{is it fair to say x and v travel are the measured displacements dur to reflex motion and then set x=0 since we have no measured displacement? "while we have no measured xtravel, simulations suggest it could be order 10 kpc."}}
  \label{fig:schematic}
\end{figure*}
...
velocities\footnote{\todo{in order to get the proper motions instead of 
velocities, we had to blahblah and blah}}
in Table~3.
