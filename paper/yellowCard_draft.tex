...
\end{align}
where ``\mwouter'' again refers to a reference frame centered at and moving with the
...
They find $\big|\vtrav\big|=32\pm4\kms$ and highest likelihood apex 
direction in Galactocentric coordinates 
...
illustration of the geometry we assume. Need this part to show what each of the vector is called in the simplified diagram, 
i.e. $\bov_{\rm obs} = \vel{M31}{\mwouter} + \vtrav + \bov_{\odot}$}


\todo{In principle, there is also a term $\vec{M31halo}{M31disk}$, 
however, there is no observational data measuring the differential motion of the
M31 disk with respect to the M31 halo, so we choose to neglect this term.
See discussion in Sec.~\ref{sec:discussion-m31}.}
...
\begin{figure*}[htb]
  ...
  \caption{
    ...
    from~\cite{Petersen2021}. \todo{change label on r}
    }
  \label{fig:schematic}
\end{figure*}
...
\todo{\apw{Add a paragraph comparing our measurements to other values (e.g., summarize aspects of your Table 3)}}
