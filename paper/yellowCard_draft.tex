...
It is not surprising that there are many discrepancies between the total and summed
values of the LG mass, since MW+M31 mass estimates often cannot measure the full
...
\todo{Our LG mass estimates are consistent with a number of other recent studies that
...
\todo{Additionally, \cite{Benisty2022} recently used the Timing Argument to place constraints on the
...
\begin{table*}
  ...
  \hline\hline
  \end{tabular}
  \caption{\label{table:masses}A collection of previous mass measurements from 
  dynamics.}
\end{table*}
...
from the proper motion measurements, which have signal-to-noise ratios
of just 3--4.
...
\todo{If we assume a MW mass of $\sim10^{12}\Msun$, 
Suddenly putting meaningful constraints on the masses of the individual systems. 
Refer to the table. }
%%%%%%%%%%%
...
MW halo to the recent infall of the LMC, as shown in  ~\cite[e.g.][]{Gomez2015,
...
We also consider the three compilations of kinematic data for the distance and 
...
\begin{itemize}
  \item[-] For the ``\textit{vdMG08 Dist. + HST PM}'' dataset -- the~\cite{vdm2008}
  distance and \textit{HST} proper motion measurements~\citep{Sohn:2012,vdm2012}
  -- we find a LG mass of 
  ...
  %
  \item[-] For the ``\textit{Cepheid Dist. + Gaia PM}'' dataset -- a combination of
  the more recent Cepheid distance and latest \textit{Gaia} proper motion 
...
\begin{itemize}
  \item \textbf{The measured travel velocity of the MW disk directly implies a reduced
  LG mass from the Timing Argument.}
  ...
  halo ($\vtrav=0$). 
  Using the same distance and PMs as in the Timing Argument model of~\cite{vdm2012}, we find that the inclusion of the travel velocity yields a reduction 
  in the LG mass of $\sim0.3\times 10^{12}\Msun$.
  %
  %
  ...
  and found an improvement in the uncertainty on the mass by a factor between
  $2-2.5\times$.
  %
  %
\end{itemize}
...
the Local Volume in the near future.
...
distances.
These endeavors will: 1. further refine our estimates of the mass of the LG to
place it in a cosmological context; and 2. permit travel velocity measurements
at the largest distances possible which will set the lower bound of LG masses
via the Timing Argument, thus placing meaningful limits on the individual masses
of the M31 and MW galaxy.
