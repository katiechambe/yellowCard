...
\kc{Other perturbations to the timing arguement: i.e. Previous works have studied other various pertubrations that one can make to the TA. For example, cosmic biad/scatter, cosmological constant, etc. Name them, their effects, and how much they contribute to TA uncertainty.}
...
relative orbits of the galaxies \citep[see, e.g.,][]{Penarrubia2014}.{\footnote{\add{Note: The effect of the Hubble flow can be included in the TA \kc{add citation}, but we ignore it for the sake of simplicity in determining the effect of only the measured travel velocity on LG mass measurements.}}
...
\add{We construct a likelihood function of measuring the observable quantities
$\vec y$ given a model with parameters $\vec K$. 
\begin{equation}
  \mathcal{L}(\vec y|\vec K)\\=
  \prod^y \frac{1}{\sqrt{2\pi \sigma_{\vec y}} } e^{ -\frac{(\vec y_K - \vec y_{\rm obs})^2}{2\sigma_{\vec y}^2} }.
\end{equation}
Here, $\vec y=\{D, \vrad, \mu^*_{\alpha}, \mu_{\delta}, t_{\rm peri}\}$ are the
measureable quantities $D$ the distance to M31, $\vrad$ the radial velocity of
M31, $\mu^*_{\alpha}$ and $\mu_{\delta}$ the proper motion components,
$t_{\rm peri}$ the time since last pericenter, and the sum is over each
observable.
$\vec K$ is a vector of given model parameters $(\mlg, a, e, \eta, \alpha)$,
where $\mlg$ is the total mass of the LG, $a$ is the semimajor-axis,
$e$ is the eccentricity, $\eta$ is the present value of the eccentric anomaly,
and $\alpha$ is a nuisance parameter, discussed in detail later in this section.
}
...
\add{We note that these values are for reference only, as we currently do not
include additional perturbations to the TA model -- such as a cosmological
constant or cosmic bias -- that may yield more
realistically representative values.}
...
\begin{table*}
  ...
  \hline\hline
  \end{tabular}
  \caption{\label{table:masses}\kc{Question \#4}A selected collection of previous mass
  measurements from LG dynamics focusing on previous TA results as well as a few
  mass measurements as discussed in the text.}
\end{table*}


\subsection{Additional perturbations to the Timing Argument}
...
In addition, \kc{Comment \#2: cosmological constant}
...
dataset: \kc{Question \#1: do we need to include errorbars on these values? How?}
 \remove{\footnote{Errors correspond to the 68\% credible regions about
...
\begin{itemize}
  ...
  \remove{LG mass of $\mlg = 3.98^{+0.63}_{-0.47}\times10^{12}~\Msun$} 
  \add{that the travel velocity decreases the inferred LG mass by 11.36\%}.
  %
  ...
  \add{that the travel velocity decreases the inferred LG mass by 12.01\%}.
  %
  ...
  measurements of M31~\citep{Li2021,Salomon2021} -- we find 
  \remove{a new measurement for the LG mass of
  $\mlg = 4.54^{+0.77}_{-0.56}\times10^{12}~\Msun$}
  \add{that the travel velocity decreases the inferred LG mass by 10.88\%}.
\end{itemize}
