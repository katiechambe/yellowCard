...
studies \citep[e.g.,][]{Kahn1959, Kroeker1991,LiWhite2008,more} \todo{TA refs}.
...
\begin{table*}
  ...
  \hline\hline
  \end{tabular}
  \tablerefs{$a.$ \cite{vdm2008}, $b.$ \cite{Courteau1999}, $c.$ \cite{vdm2012}, $d.$ \cite{Schonrich2010,McMillan2011}, $e.$ \cite{Jarosik2011}, $f.$ \cite{Li2021},$g.$ \citep{Salomon2021}, $h.$ \citep{Drimmel2018}, $i.$ \cite{Planck2018}}
...
\begin{figure*}[htb]
  ...
  ~\cite{Salomon2021} yield a more circular orbit, and thus a lower eccentricity
   ...
   \apw{One more idea: Can we add the numerical values of our mass constraints in a text box in the lower left panel? I want to see median with +/- values that contain 68\% of the marginal posterior mass. You computed these from percentiles before, I think, from getting [16, 50, 84]th percentiles. Can we put the 3 median posterior masses and confidence intervals as text in the lower left of the lower left panel?}
   }
...
To summarize: Using the same dataset as used in \citet{vdm2012}, we find a
median posterior LG mass of $\mlg = XX ^{+YY}_{-ZZ}~\Msun$ (where error bars
here report the 68\% confidence interval around the median).
\apw{We should report median and percentile errorbars! Can you fill in the values? Are these still larger than the vdm2012 values? Also add sentences for the other two datasets}
...
\begin{figure}[htb]
    ...
    \apw{Can you change from mean to median? Also I think we can remove the markers at vtravel=32 - I think the point of this figure is to show the scaling. You could just put a vertical dashed line at 32 to highlight where the other curves intersect.}
    }
  \end{figure}
...
leading to a slightly higher inferred Local Group mass $\mlg = XX$.
