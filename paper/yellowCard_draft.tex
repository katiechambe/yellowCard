...
For each dataset, we run the sampler with XX chains for 4000 tuning steps and
2000 draws.
% TODO: let's up this to 4 chains, 4000 tuning, 8000 draws
...
It is likely that the close passage of M33 is inducing a
reflex motion of the M31 disk \citep{?}. Our model assumes that the disk of 
M31 is moving coherently with its outer halo, but if the disk is also 
experiencing its own reflex motion, then the measured velocities of the M31 
disk will differ from those of the halo. This is the same effect that we have 
accounted for with the MW but for the M31 system. 
 % need to find proper reference for this
We have neglected any effects of the reflex motion of M31, as simulations of the M31--M33 system from \cite{Patel2017} and \cite{} imply that M33 is currently on its closest approach to M31. As M33 has likely not undergone a pericentric passage of M31, any induced reflex motion would be minimal and subdominant to that of the MW disk. 
